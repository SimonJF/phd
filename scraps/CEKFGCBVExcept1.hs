...
data ContinuationFrame = Frame (Environment, Var, Term) | Marker
  deriving (Show)
...
unwindStack :: Continuation -> CEK Continuation
unwindStack [] = throwError "Unhandled exception!"
unwindStack (Marker : k) = return k
...
step ((TReturn tv), e, (Frame (e', x, t)) : k) =
  ...
  return (t, e'', k)
step (TReturn tv, e, Marker : k) = return ((TReturn tv), e, k)
step ((THere t), e, k) = return (t, e, Marker : k)
step ((TGo tv), e, k) = unwindStack k >>= \k' -> return $ (TReturn tv, e, k')
...
tGoExample :: Term
tGoExample =
  TLet "x" (THere (TLet "z" (TGo (VInt 5)) (TReturn $ VInt 10))) (TReturn $ VVar "x")
