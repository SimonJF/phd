...
data ContinuationFrame = Frame (Environment, Var, Term) | ExnFrame (Environment, Var, Term, Term)
  deriving (Show)
...
add :: TermValue -> TermValue -> Term
add i1 i2 = TApp (VBuiltin Add) [i1, i2]
...
unwindStack :: Continuation -> CEK (Term, Environment, Continuation)
unwindStack [] = throwError "Unhandled exception!"
unwindStack ((ExnFrame (e, x, _, failure)) : k) = return (failure, e, k)
unwindStack (_ : k) = unwindStack k
...
-- value.
step ((TReturn tv), e, (Frame (e', x, t)) : k) =
  ...
  return (t, e'', k)
step (TReturn tv, e1, (ExnFrame (e2, x, success, _)) : k) =
  interpValue tv e1 >>= \mv ->
  let e2' = Map.insert x mv e2 in
  return (success, e2', k)
step ((TTry t x success failure), e, k) = return (t, e, (ExnFrame (e, x, success, failure)) : k )
step (TThrow, e, k) = unwindStack k
...
termTry1 =
  TTry (
    TLet "x" (TReturn (VInt 5)) (
      TLet "y" (TReturn (VInt 10)) (add (VVar "x") (VVar "y")))
  ) "x" (TReturn (VVar "x")) (TReturn (VInt (-100)))
...
termTry2 =
  TTry (
    TLet "x" (TReturn (VInt 5)) (
      TLet "y" 
        (TTry (TThrow) "x" (TReturn (VVar "x")) (TReturn (VInt 100)))
        (add (VVar "x") (VVar ("y"))))
  ) "x" (TReturn $ VVar "x") (TReturn $ VInt (-1))
